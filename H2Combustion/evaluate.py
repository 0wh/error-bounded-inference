...
def test(compBoundLinfMDR, quantizedModels, path_to_input_tensor_file, path_to_reduced_data_folder, compressor='zfp'):
...
def evaluate(data_dir): #path_to_H2Combustion_dataset
...
if __name__ == "__main__":
    ...
    parser.add_argument("--reduced", required=True, help="Path to the reduced tensor file")
    ...
    for ref in ['zfp_re', 'sz_re', 'mgard_re', 'sz_re_r2', 'mgard_re_r2']:
        ...
        inputs = torch.load(args.input)
        outputs = model(inputs).detach()
        compressor = 'zfp' #valid values: 'zfp', 'sz', 'mgard'
        for tol, model in zip(_compBound, quantizedModels):
            try:
                inputs_lossy = torch.load(f'{args.reduced}/{compressor}_{tol}', dtype=torch.float32)
            except:
